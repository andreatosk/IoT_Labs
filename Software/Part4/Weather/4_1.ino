...
#define capacity JSON_OBJECT_SIZE(4)+120
...
boolean gotMeteo=false;


//CURL PER METEO

void elaborateWeather(String weather_url){
      /*Invoca il servizio weather esposto da cherrypy, quindi non c'è un url vero e proprio
    in ogni caso
    Va fatta una GET sul <baseurl>, senza path o query
    Ci sarebbe anche la possibilità di richiedere esplicitamente la creazione di una nuova sessione
    da parte del server (che di default ne crea una nuova ogni 24h oppure ogni volta che l'arduino 
    cambia indirizzo ip) all'url <baseurl>/new.
    Potrebbe avere senso effettuare una richiesta a questo url ad ogni accensione dell'arduino, ma 
    non è strettamente necessario perché il server può gestire le sessioni in automatico
    La GET ritorna un JSON (serializzato) nel formato
      {
        "temperature":"value + 'Cel'",
        "wind":"value + 'm/s' + direction",
        "weather":"current_weather",
        "humidity":"value + '%'"
      } (i simboli + non ci sono veramente)
    Alla fine, deve stampare questi dati sul display, probabilmente in due schermate perché una non credo basti
    */
    String weather = my_curl("localhost");
    //Deserializzo il json
    DeserializationError err = deserializeJson(doc_rec,weather);
    if(err){
    ...
    strcpy(outCel,doc_rec[0]["temperature"]);
    strcpy(wind,doc_rec[0]["wind"]);
    strcpy(weather,doc_rec[0]["weather"]);
    strcpy(humidity,doc_rec[0]["humidity"]);
    /*
    outCel = doc_rec[0]["temperature"]
    wind = doc_rec[0]["wind"]
    weather = doc_rec[0]["weather"]
    humidity = doc_rec[0]["humidity"]
    */
}


String my_curl( String url)
{
  Process p;
...
void lcdShow() {
  ...
  lcd_timer = (lcd_timer - (timer + t1)) / 1000;
  if (lcd_timer % 2 == 0) {
